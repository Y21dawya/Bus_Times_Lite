...
def get_naptan(method, stop_data):
    stop_ID = ""
    if method == "1":
        stop_ID = stop_data
    elif method == "2":
    #    sms_to_naptan(stop_data)
        stop_ID = stops.execute(f'SELECT Naptan_Atco FROM bus_stops WHERE Stop_Code = {stop_data}').fetchone()
    ...
    #    stop_search(stop_data)
        potential = stops.execute(f'SELECT Naptan_Atco, Heading, Stop_Name FROM bus_stops WHERE Stop_Name LIKE "%{stop_data}%"').fetchall()
        #print(potential)
        for i in range (0,len(potential)):
            print(f"{i+1} - {potential[i]}")
...
filter_method = input("What filtering method would you like to use?\n To filter by a specific bus, enter 1.\n To filter by buses calling subsequently at a given stop, enter 2.\n If none, leave blank\n").strip()
...
if response.status_code == 200:
    ...
    for bus in data:
        ...
        elif bus['lineName'] in search_bus:
            print(f"{bus['modeName']} {bus['lineName']} to {bus['destinationName']} arriving in {bus['timeToStation'] // 60} minutes, {bus['timeToStation'] % 60} seconds.")
else:
